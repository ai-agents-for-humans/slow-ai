...
def _try_parse_brief(text: str) -> "ProblemBrief | None":
    """Extract and parse a ProblemBrief from a raw string output.

    Handles the case where a model returns JSON as plain text instead of
    using the structured output tool call.
    """
    ...
    for candidate in candidates:
        try:
            return ProblemBrief.model_validate_json(candidate)
        except Exception:
            continue
    return None
...
async def interview_message(
    ...
    if isinstance(output, str):
        parsed = _try_parse_brief(output)
